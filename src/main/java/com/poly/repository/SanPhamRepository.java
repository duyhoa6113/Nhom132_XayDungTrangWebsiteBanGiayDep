...
public interface SanPhamRepository extends JpaRepository<SanPham, Integer>,
     ...
     */
    @Query("SELECT COUNT(sp) FROM SanPham sp WHERE sp.trangThai = 1")
    Long countActiveProducts();

    /**
     * Lấy danh sách sản phẩm theo trạng thái (không phân trang) - cho admin
     */
    @Query("SELECT sp FROM SanPham sp WHERE sp.trangThai = :trangThai ORDER BY sp.createdAt DESC")
    List<SanPham> findByTrangThai(@Param("trangThai") int trangThai);
}